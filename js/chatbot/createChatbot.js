...
export async function createChatBot(chatData) {
	...
	let searchEvaluationInChatData = function(table, tagWord) {
		const tailleTable = table.length ;
		for (let i = 0; i < tailleTable ; i++) {
			if (table[i][0] == tagWord)
				return table[i][4] ;
		}
		return [] ;
	}
	...
	let organiseCompetences = function(data) {
		...
		for (let i=0; i < nbItems; i++) {
			...
			let cle = data[0] ;
			console.log(cle) ;
			if (result[cle] === undefined) {
				result[cle] = [] ;
			}
			result.push(result[cle])
	...
	let getEvaluationItems = function() {
		let textEvaluation = document.getElementById("evaluation").value ;
		if (textEvaluation == "") {
			return [] ;
		} 
		...
		textEvaluation = textEvaluation.substring(1) ;
		let table = textEvaluation.split("#") ;
		let nbElementsTable = table.length ;
		for (let i=0;i<nbElementsTable;i++) {
			let data = searchEvaluationInChatData(chatData, table[i]) ;
			if (data.length)
				response.push(data)

		}
		return organiseCompetences(response) ;
	}
	...
	document.body.addEventListener('click', function(event) {
		event.preventDefault() ;
	    if (event.target.classList.contains('gen_pdf')) {
	    	// Si la réponse est OK, déclenche le téléchargement du PDF
	       	getPdf() ;
	    }
	});
	...
	function handleClick(event) {
		...
		if (target.tagName === "A") {
			...
			const valeur = document.getElementById("evaluation").value	; 
			document.getElementById("evaluation").value +=  link ; 
